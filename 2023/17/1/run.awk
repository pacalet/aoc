...
# direction "dir". If the step is valid (no return, no more than nmax steps in
# same direction, final position on grid), and the adjacent vertex has not
# already been visited, update "losses" and "visited". Plus, if the new position
# is "[height,width]", print the final loss and stop.
function go(v, dir, loss,    va, x, y, d, n) {
	debug::debug("go(" v ", " sdir[dir] ") with loss " loss)
	split(v, va, SUBSEP); x=va[1]; y=va[2]; d=va[3]; n=va[4]
	# New coordinates, number of steps in same direction and loss
	x+=off["x",dir]; y+=off["y",dir]; n=(dir==d)?n+1:1; loss+=grid[x,y]
	# If valid step and adjacent vertex not visited
	if((x>=1) && (x<=width) && (y>=1) && (y<=height) && (n<=nmax) && (dir!=op[d]) && (!((x,y,dir,n) in visited))) {
		debug::debug(": OK\n")
		losses[loss][x,y,dir,n]; visited[x,y,dir,n]
		if((x==width) && (y==height)) { print loss; exit }
	} else { debug::debug(": KO\n") }
}

BEGIN {
	# Maximum consecutive steps in same direction
	nmax=3
	# Directions
